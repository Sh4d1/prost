...
fn try_message(input: TokenStream) -> Result<proc_macro2::TokenStream, Error> {
...
fn strip_prost_messages_from_procmacro_attrs(
...
pub fn open_oneof(_attr: TokenStream, item: TokenStream) -> TokenStream {
    let methods = try_message(item.clone()).expect("failed to compute methods");
